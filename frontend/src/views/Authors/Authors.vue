<template>
  <div v-if="construction" class="md:flex grid grid-cols-2 gap-4 container mx-auto px-4">
    <div v-for="author in authors" :key="author.id" class="md:mr-6 px-6 py-3 flex items-center justify-center border rounded-sm border-darkGrey">
        <router-link :to="{name: 'authorBlogs', params: {id: author.id}}">{{ author.user }} {{ author.user.last_name }}</router-link>
    </div>
  </div>
  <div v-else class="flex container flex-wrap max-w-md gap-x-4 gap-y-10 mx-auto px-4">
    ...
    <p>This page is currently undergoing sheduled maintenance. We will be back shortly. Thank you for your patience.</p>
